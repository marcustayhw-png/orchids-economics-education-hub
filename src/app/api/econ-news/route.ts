  ...
  try {
    ...
    const newNews = await db.insert(econNews)
      .values({
        ...
        theoryConnection: theoryConnection?.trim(),
  ...
  try {
    ...
    const updatedNews = await db.update(econNews)
      .set({
        ...
        context: context?.trim() ?? existingNews[0].context,
        explanation: explanation?.trim() ?? existingNews[0].explanation,
        theoryConnection: theoryConnection?.trim() ?? existingNews[0].theoryConnection,

...
export default function EconNewsPage() {
  ...
  const filteredNews = selectedLevel === "both" 
    ? news 
    : news.filter(item => item.level === selectedLevel || item.level === "Both");
    ...
    <div className="min-h-screen bg-background py-8 sm:py-12 px-4 sm:px-6 lg:px-8 overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <Tabs defaultValue="both" className="space-y-8 sm:space-y-10 w-full" onValueChange={setSelectedLevel}>
          <TabsList className="flex flex-col sm:grid sm:grid-cols-3 w-full max-w-2xl mx-auto h-auto sm:h-11 p-1 gap-1 bg-muted/50 backdrop-blur-sm">
            <TabsTrigger 
              value="both" 
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <CardTitle className="text-2xl sm:text-3xl font-bold break-words leading-tight tracking-tight">
                            {item.title}
                          </CardTitle>
                        </div>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {item.topics.map((topic, idx) => (
                            <Badge 
                              key={idx} 
                              variant="secondary" 
                              className="text-xs font-medium px-3 py-1 bg-primary/10 hover:bg-primary/20 transition-colors"
                            >
                              {topic}
                            </Badge>
                          ))}
                      ...
                      <div className="space-y-3 pt-2 border-t border-border/30">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {item.theories.map((theory, idx) => (
                            <Badge 
                              key={idx} 
                              className="text-xs font-medium px-3 py-1 bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 transition-all"
                            >
                              {theory}
                            </Badge>
                          ))}
